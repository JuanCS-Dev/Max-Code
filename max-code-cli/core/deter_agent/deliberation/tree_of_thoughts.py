...
class TreeOfThoughts:
    ...
    def _default_generator(
        ...
        """
        Generator padrão com fallback inteligente (pattern-based)

        Quando LLM não está disponível, usa pattern matching para gerar
        planos básicos mas REAIS baseados em palavras-chave.

        Em produção com LLM, isso seria substituído por prompt engineering.
        """
        ...
        # Pattern 1: API (REST vs GraphQL)
        if any(keyword in problem_lower for keyword in ['api', 'rest', 'graphql', 'endpoint']):
            if 'graphql' in problem_lower:
                thoughts.append(Thought(
                    id="thought_graphql",
                    description="GraphQL API with Apollo Server",
                    approach="Use Apollo Server with type-safe schema. Single endpoint /graphql with query/mutation support. Implement resolvers for data fetching.",
                    pros=["Single endpoint", "Type-safe", "Flexible queries", "No over-fetching"],
                    cons=["Learning curve", "Complexity for simple use cases", "Caching harder"],
                    assumptions=["Team knows GraphQL", "Frontend needs flexible queries"],
                    risks=["Over-engineering for simple CRUD"],
                    complexity="MEDIUM",
                    implementation_plan=[
            ...
            if 'rest' in problem_lower or 'api' in problem_lower:
                thoughts.append(Thought(
                    id="thought_rest",
                    description="RESTful API with Express/FastAPI",
                    approach="Traditional REST endpoints with HTTP verbs (GET/POST/PUT/DELETE). Resource-based routing. OpenAPI/Swagger docs.",
                    pros=["Simple", "Widely understood", "Good caching", "Stateless"],
                    cons=["Over-fetching", "Multiple endpoints", "Versioning complexity"],
                    assumptions=["Standard CRUD operations", "HTTP-based clients"],
                    risks=["Endpoint proliferation"],
                    complexity="LOW",
                    implementation_plan=[
                        "Define resource endpoints (e.g., /users, /posts)",
                        "Implement CRUD routes with proper HTTP methods",
        ...
        if any(keyword in problem_lower for keyword in ['auth', 'login', 'user', 'security']):
            thoughts.append(Thought(
                id="thought_jwt",
                description="JWT-based authentication",
                approach="Use JSON Web Tokens for stateless auth. Login returns token, protected routes verify token.",
                pros=["Stateless", "Scalable", "Cross-domain", "Mobile-friendly"],
                cons=["Token revocation hard", "Storage concerns", "Size overhead"],
                assumptions=["HTTPS available", "Token rotation handled"],
                risks=["XSS if stored in localStorage"],
                complexity="MEDIUM",
                implementation_plan=[
        ...
        if any(keyword in problem_lower for keyword in ['database', 'storage', 'data', 'crud']):
            thoughts.append(Thought(
                id="thought_sql",
                description="SQL database (PostgreSQL/MySQL)",
                approach="Relational database with ACID guarantees. Use ORM (Prisma/SQLAlchemy) for type safety.",
                pros=["ACID", "Complex queries", "Mature", "Relational integrity"],
                cons=["Schema migrations", "Scaling complexity", "Rigid structure"],
                assumptions=["Structured data", "Complex relationships"],
                risks=["Performance at scale"],
                complexity="MEDIUM",
                implementation_plan=[
                    "Design database schema (entities + relationships)",
                    "Setup ORM and migration tool",
        ...
        # Pattern 4: Generic function/algorithm
        if any(keyword in problem_lower for keyword in ['function', 'calculate', 'algorithm', 'code']):
            thoughts.append(Thought(
                id="thought_simple",
                description="Simple function with clear inputs/outputs",
                approach="Single-purpose function with type hints, validation, and comprehensive tests.",
                pros=["Simple", "Testable", "Reusable", "Easy to understand"],
                cons=["Limited scope", "May need refactoring as requirements grow"],
                assumptions=["Requirements well-defined", "Pure function preferred"],
                risks=["Over-simplification"],
                complexity="LOW",
                implementation_plan=[
        ...
        # Se nenhum pattern detectado, gerar plano genérico
        if not thoughts:
            thoughts.append(Thought(
                id="thought_generic",
                description=f"Standard implementation approach for: {problem[:50]}...",
                approach="Break down problem into smaller steps. Implement incrementally with tests. Follow best practices.",
                pros=["Structured approach", "Testable", "Maintainable"],
                cons=["Generic solution may miss specific optimizations"],
                assumptions=["Standard tools available", "Time for proper implementation"],
